...
function verificaColisao2(){
  ...
  for(let i=0; i < imagemDoCarro.length; i++){
    ...
    colidiu = collideRectCircle(xCarros[i],yCarros[i], comprimetoCarros[i], altura, xAtor2, yAtor2,15);
...
{ 
  colidiu = colidiu = collideRectCircle(xCarros[i],yCarros[i], comprimetoCarros[i], altura, xAtor, yAtor,15);
    if(colidiu){
      console.log("colidiu");
      voltaAtorParaPosicaoInicial();
      somColidiu.play();
